...
#[cfg(target_os = "linux")]
#[gpui::test]
fn download_samples_progress_installs_and_releases_task(cx: &mut TestAppContext) {
...
#[cfg(target_os = "linux")]
#[test]
fn official_restart_handoff_uses_installed_path_and_skips_none_or_error() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path().join("demo");
    let archive = dir.path().join("download.tar.gz");
    std::fs::write(&archive, tarball(dir.path())).unwrap();
    let installed = install::install(&archive, &root);
    let mut path = None;
    let version = Version::new(4, 0, 0);
    let status = installed_status(installed, version.clone(), |p| path = Some(p));
    assert_eq!(path, Some(root));
    assert_eq!(status, UpdateStatus::Staged(version.clone()));
    assert_eq!(
        installed_status(
            Ok(Installed { restart_path: None }),
            version.clone(),
            |_| panic!("no handoff expected")
        ),
        UpdateStatus::Staged(version.clone())
    );
    assert_eq!(
        installed_status(Err(Error::Install("fixture".into())), version, |_| panic!(
            "no handoff expected"
        )),
        UpdateStatus::Errored("install failed: fixture".into())
    );
}
